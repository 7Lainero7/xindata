...
def analyze(query):
    """
    Анализирует введённый пользователем вопрос, генерирует соответствующий SQL-запрос с помощью LLM, выполняет его и выводит результат.

    :param query: str, вопрос на естественном языке, который требуется преобразовать в SQL-запрос и выполнить.
